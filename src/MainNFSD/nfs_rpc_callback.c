...
int nfs_rpc_create_chan_v40(nfs_client_id_t *clientid, uint32_t flags)
{
	...
	case IPPROTO_UDP:
		...
		chan->clnt = clnt_dg_ncreate(fd, &raddr,
					     clientid->cid_cb.v40.cb_program,
					     NFS_CB /* Errata ID: 2291 */,
					     0, 0);
		break;
	...
	if (chan->clnt->cl_error.re_status != RPC_SUCCESS) {
		err = rpc_sperror(&chan->clnt->cl_error, __func__);
	...
	if (chan->auth->ah_error.re_status != RPC_SUCCESS) {
		err = rpc_sperror(&chan->auth->ah_error, __func__);
