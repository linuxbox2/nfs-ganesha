			    ...
			    const char *path, struct fsal_obj_handle **handle)
{
	...
	struct glusterfs_namespace *glfs_namespace =
	    container_of(parent->namespace, struct glusterfs_namespace, namespace);
	...
	glhandle =
	    glfs_h_lookupat(glfs_namespace->gl_fs, parenthandle->glhandle, path,
			    &sb);
	if (glhandle == NULL) {
		status = gluster2fsal_error(errno);
		goto out;
				  ...
				  fsal_readdir_cb cb, bool * eof)
{
	...
	struct glusterfs_namespace *glfs_namespace =
	    container_of(dir_hdl->namespace, struct glusterfs_namespace, namespace);
	struct glusterfs_handle *objhandle =
	...
	glfd = glfs_h_opendir(glfs_namespace->gl_fs, objhandle->glhandle);
	if (glfd == NULL) {
			    ...
			    struct fsal_obj_handle **handle)
{
	...
	rc = setglustercreds(glfs_namespace, &opctx->creds->caller_uid,
	...
	glhandle =
	    glfs_h_creat(glfs_namespace->gl_fs, parenthandle->glhandle, name,
			 O_CREAT, fsal2unix_mode(attrib->mode), &sb);

	rc = setglustercreds(glfs_namespace, NULL, NULL, 0, NULL);
	if (rc != 0) {
		status = gluster2fsal_error(EPERM);
		LogFatal(COMPONENT_FSAL, "Could not set Ganesha credentials");
		goto out;
			     ...
			     struct fsal_obj_handle **handle)
{
	...
	/* FIXME: what else from attrib should we use? */
	glhandle =
	    glfs_h_mkdir(glfs_namespace->gl_fs, parenthandle->glhandle, name,
			      ...
			      struct fsal_obj_handle **handle)
{
	...
	/* FIXME: what else from attrib should we use? */
	glhandle =
	    glfs_h_mknod(glfs_namespace->gl_fs, parenthandle->glhandle, name,
				 ...
				 struct fsal_obj_handle **handle)
{
	...
	/* FIXME: what else from attrib should we use? */
	glhandle =
	    glfs_h_symlink(glfs_namespace->gl_fs, parenthandle->glhandle, name,
			   link_path, &sb);
				 ...
				 bool refresh)
{
	...
	fsal_status_t status = { ERR_FSAL_NO_ERROR, 0 };
	struct glusterfs_namespace *glfs_namespace =
	    container_of(obj_hdl->namespace, struct glusterfs_namespace, namespace);
	struct glusterfs_handle *objhandle =
	    container_of(obj_hdl, struct glusterfs_handle, handle);
#ifdef GLTIMING
	struct timespec s_time, e_time;
	...
	rc = glfs_h_readlink(glfs_namespace->gl_fs, objhandle->glhandle,
			      ...
			      const struct req_op_context *opctx)
{
	...
	if (objhandle->openflags != FSAL_O_CLOSED) {
		...
		rc = glfs_h_stat(glfs_namespace->gl_fs, objhandle->glhandle, &sb);
	}
			      ...
			      struct attrlist *attrs)
{
	...
	if (FSAL_TEST_MASK(attrs->mask, ATTR_SIZE)) {
		rc = glfs_h_truncate(glfs_namespace->gl_fs, objhandle->glhandle,
				     attrs->filesize);
		if (rc != 0) {
			status = gluster2fsal_error(errno);
			goto out;
	...
	rc = glfs_h_setattrs(glfs_namespace->gl_fs, objhandle->glhandle, &sb,
			     mask);
	if (rc != 0) {
		status = gluster2fsal_error(errno);
		goto out;
			      ...
			      const char *name)
{
	...
	credrc =
	    setglustercreds(glfs_namespace, &opctx->creds->caller_uid,
			    &opctx->creds->caller_gid,
			    opctx->creds->caller_glen,
			    opctx->creds->caller_garray);
	if (credrc != 0) {
	...
	rc = glfs_h_link(glfs_namespace->gl_fs, objhandle->glhandle,
			 dstparenthandle->glhandle, name);

	credrc = setglustercreds(glfs_namespace, NULL, NULL, 0, NULL);
	if (credrc != 0) {
		status = gluster2fsal_error(EPERM);
		LogFatal(COMPONENT_FSAL, "Could not set Ganesha credentials");
		goto out;
				...
				const char *new_name)
{
	...
	fsal_status_t status = { ERR_FSAL_NO_ERROR, 0 };
	struct glusterfs_namespace *glfs_namespace =
	    container_of(olddir_hdl->namespace, struct glusterfs_namespace, namespace);
	...
	rc = glfs_h_rename(glfs_namespace->gl_fs, srcparenthandle->glhandle,
				 ...
				 const char *name)
{
	...
	rc = glfs_h_unlink(glfs_namespace->gl_fs, parenthandle->glhandle, name);
			       ...
			       fsal_openflags_t openflags)
{
	...
	glfd = glfs_h_open(glfs_namespace->gl_fs, objhandle->glhandle, p_flags);
	if (glfd == NULL) {
		status = gluster2fsal_error(errno);
		goto out;
	}
