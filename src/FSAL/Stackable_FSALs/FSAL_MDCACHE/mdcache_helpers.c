			 ...
			 fsal_cookie_t cookie, fsal_cookie_t *ret_cookie)
{
	...
	LogFullDebug(COMPONENT_CACHE_INODE,
		     "Creating cache entry for %s last_ck=0x%"PRIx64
		     " cookie=0x%"PRIx64" sub_handle=0x%p",
		     name, state->last_ck, cookie, sub_handle);
	...
	new_dir_entry->ck = state->last_ck;
	new_dir_entry->nk = cookie;
	...
	if (glist_null(&new_dir_entry->chunk_list)) {
		 ...
		 *
		 * Remember this cookie for the next entry and add this entry
		 * to the chunk.
		 */
		state->last_ck = cookie;
					 ...
					 mdcache_dir_entry_t **dirent)
{
	...
	state.last_ck = whence; /* Need cookie that finds each dirent */
	...
	if (chunk->num_entries == 0) {
		...
		*dirent = glist_first_entry(&chunk->dirents,
		...
		if (eod) {
			/* If end of directory, make sure cookie for last
			 * entry is LAST_COOKIE.
			 */
...
again:
	...
	LogFullDebug(COMPONENT_NFS_READDIR,
		     "Readdir chunked next_ck=0x%"PRIx64, next_ck);

	if (mdcache_avl_lookup_ck(directory, next_ck, &dirent) ==
	    MDCACHE_AVL_NOT_FOUND) {
		fsal_status_t status;
		...
		LogFullDebug(COMPONENT_NFS_READDIR,
			     "Readdir chunked about to populatenext_ck=0x%"
			     PRIx64, next_ck);
		 ...
		 */
		status =
		    mdcache_populate_dir_chunk(directory, next_ck, &dirent);
	...
	for (;
	     dirent != NULL;
	     dirent = glist_next_entry(&dirent->chunk->dirents,
				       mdcache_dir_entry_t,
				       chunk_list,
				       &dirent->chunk_list)) {
		fsal_status_t status;
		...
		if (cb_result >= DIR_MARK ||
		    next_ck == 0 || next_ck == LAST_COOKIE) {
			 ...
			 * end of directory (for DIR_MARK, we expect the
			 * callback DID consume the last entry).
			 */
			*eod_met = (next_ck == 0 || next_ck == LAST_COOKIE) &&
				   cb_result != DIR_TERMINATE;
