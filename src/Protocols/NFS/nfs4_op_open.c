		 ...
		 struct nfs_resop4 *resp)
{
	...
	case CLAIM_DELEGATE_CUR:
		...
		if (!data->export->namespace->ops->fs_supports(
				data->export->namespace, fso_delegations)) {
	...
	if (data->export->namespace->ops->
	    fs_supports(data->export->namespace, fso_delegations)
