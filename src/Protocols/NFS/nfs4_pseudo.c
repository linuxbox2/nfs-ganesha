			   ...
			   cache_entry_t *entry)
{
	...
	if (parent_entry == op_ctx->ctx_export->exp_root_cache_inode) {
		LogDebug(COMPONENT_EXPORT,
...
retry:
	...
	if (strcmp(op_ctx->ctx_export->fsal_export->fsal->name,
		   "PSEUDO") != 0) {
...
bool pseudo_mount_export(struct gsh_export *export)
{
	...
	*last_slash = '/';
	...
	cache_status = nfs_export_get_root_entry(op_ctx->ctx_export,
						 &state.dirent);
